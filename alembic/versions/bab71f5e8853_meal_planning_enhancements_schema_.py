...
def upgrade() -> None:
    ...
    with op.batch_alter_table("meals", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column(
                "is_shopped", sa.Boolean(), nullable=False, server_default=sa.text("0")
            )
        )
    ...
    op.execute(
        "UPDATE meals SET status = 'Queued' WHERE status IN ('Draft', 'Scheduled')"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE meals SET status = 'Draft' WHERE status IN ('Queued', 'Cancelled')"
    )
