...
from app.models import Recipe, Meal, MealStatus, User
...
def test_migrate_meals_status_logic(db, mock_session_local):
    ...
    df_recipes_old = pd.DataFrame([{"Recipe_ID": 101, "Recipe_Name": "Test Recipe"}])
    ...
    meal_past = (
        db.query(Meal)
        .filter(not Meal.name.like("%Unknown Date%"))
        .filter(Meal.date < datetime.now())
        .first()
    )
