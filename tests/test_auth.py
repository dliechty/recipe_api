...
def test_admin_promotion(client: TestClient, db):
    ...
    res = client.put(
        f"/auth/users/{user_id}", headers=user_headers, json={"is_admin": True}
    )
    assert res.status_code == 403

    # 2. Admin promotes user (Should Succeed)
    res = client.put(
        f"/auth/users/{user_id}", headers=admin_headers, json={"is_admin": True}
    )
    assert res.status_code == 200
    assert res.json()[
        "is_admin"
    ]  # Depends if UserPublic shows it, actually it serves UserPublic which currently doesn't have is_admin
