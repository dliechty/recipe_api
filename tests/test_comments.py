...
def test_update_comment_permissions(client, db):
    ...
    res = client.post(f"/recipes/{recipe_id}/comments", json=comment_data, headers=commenter_headers)
    comment_id = res.json()["id"]

    # 3. Commenter updates own comment -> OK
    update_data = {"text": "Updated by owner"}
    res = client.put(f"/recipes/{recipe_id}/comments/{comment_id}", json=update_data, headers=commenter_headers)
    ...
    res = client.put(f"/recipes/{recipe_id}/comments/{comment_id}", json={"text": "Hacked"}, headers=stranger_headers)
    assert res.status_code == 403

    # 5. Recipe Owner tries to update -> Forbidden (unless admin)
    res = client.put(f"/recipes/{recipe_id}/comments/{comment_id}", json={"text": "Owner Override"}, headers=owner_headers)
    assert res.status_code == 403

    # 6. Admin updates -> OK
    admin_headers = get_auth_headers(client, db, email="admin_comment@example.com", is_admin=True)
    res = client.put(f"/recipes/{recipe_id}/comments/{comment_id}", json={"text": "Admin Override"}, headers=admin_headers)
    assert res.status_code == 200
...
def test_delete_comment_permissions(client, db):
    ...
    res = client.post(f"/recipes/{recipe_id}/comments", json={"text": "To be deleted"}, headers=commenter_headers)
    comment_id = res.json()["id"]

    # 2. Stranger delete -> Forbidden
    stranger_headers = get_auth_headers(client, db, email="stranger_del@example.com")
    res = client.delete(f"/recipes/{recipe_id}/comments/{comment_id}", headers=stranger_headers)
    assert res.status_code == 403

    # 3. Recipe Owner delete -> Forbidden
    res = client.delete(f"/recipes/{recipe_id}/comments/{comment_id}", headers=owner_headers)
    assert res.status_code == 403
    ...
    res = client.post(f"/recipes/{recipe_id}/comments", json={"text": "To be deleted by self"}, headers=commenter_headers)
