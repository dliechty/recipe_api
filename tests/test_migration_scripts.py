...
from migration_scripts import (
    purge_recipes,
    purge_meals,
    migrate_access_recipes,
    migrate_access_meals,
)
...
def test_purge_recipes(db, mock_session_local):
    ...
    with patch(
        "migration_scripts.purge_recipes.SessionLocal", side_effect=mock_session_local
    ):
...
def test_purge_meals(db, mock_session_local):
    ...
    with patch(
        "migration_scripts.purge_meals.SessionLocal", side_effect=mock_session_local
    ):
...
def test_migrate_recipes(db, mock_session_local):
    # Setup User
    user = User(
        email="admin@example.com", hashed_password="pw", is_admin=True, is_active=True
    )
    db.add(user)
    db.commit()
        ...
        [
            {
                ...
                "Recipe_Servings": 4,
                "Recipe_Prep_Time": "10min",
                "Recipe_Cook_Time": "20min",
    ...
    with (
        patch(
            "migration_scripts.migrate_access_recipes.SessionLocal",
            side_effect=mock_session_local,
        ),
        patch(
            "migration_scripts.migrate_access_recipes.run_mdb_export",
            side_effect=mock_run_mdb_export,
        ),
...
def test_migrate_meals(db, mock_session_local):
        ...
        [
            {
                "Meal_Template_ID": 5,
                "Meal_Type_ID": 3,
                "Meal_Template_Name": "Test Template",
            }
        ]
    )
    ...
    with (
        patch(
            "migration_scripts.migrate_access_meals.SessionLocal",
            side_effect=mock_session_local,
        ),
        patch(
            "migration_scripts.migrate_access_meals.run_mdb_export",
            side_effect=mock_run_mdb_export,
        ),
        patch("os.path.exists", return_value=True),
    ):
    ...
    template = (
        db.query(MealTemplate).filter(MealTemplate.name == "Test Template").first()
    )
    ...
    # Check slots
    direct_slot = next(
        s for s in template.slots if s.strategy == MealTemplateSlotStrategy.DIRECT
    )
    search_slot = next(
        s for s in template.slots if s.strategy == MealTemplateSlotStrategy.SEARCH
    )

    assert direct_slot.recipe.name == "Test Recipe"
    assert search_slot.search_criteria == [
