...
def get_auth_headers(client: TestClient, db, email="user_sorting@example.com", password="password"):
    try:
...
def test_recipes_sorted_by_name(db):
    ...
    assert recipe_names == sorted(recipe_names), "Recipes should be sorted by name alphabetically"
...
def test_multi_field_sorting(client: TestClient, db):
    ...
    # We expect R2, R3, R1
    expected = ["R2", "R3", "R1"]
    assert names == expected, f"Sorting by category failed. Expected {expected}, got {names}"
    ...
    expected = ["R1", "R3", "R2"]
    assert names == expected, f"Sorting by cuisine failed. Expected {expected}, got {names}"
